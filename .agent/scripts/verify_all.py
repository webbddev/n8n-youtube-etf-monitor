...
def run_script(name: str, script_path: Path, project_path: str, url: Optional[str] = None) -> dict:
    ...
    try:
        ...
        if passed:
            ...
            if result.stderr:
                stderr_preview = str(result.stderr)[:300]
...
Examples:
  ...
  python scripts/verify_all.py . --url https://staging.example.com --no-e2e
    ...
    for suite in VERIFICATION_SUITE:
        ...
        category_str = str(suite["category"])
        print_header(f"📋 {category_str.upper()}")
        
        for name, script_path, required in suite["checks"]: # type: ignore
